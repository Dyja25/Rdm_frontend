...
function CandidateGreenTable(props) {
  const [page, setPage] = useState(0);
   const [hasMore, setHasMore] = useState(true);
  useEffect(() => {
    //props.getCandidateListByCategory("blue");
    props.getCandidateListByCategory("Green",page,props.userId);
    setPage(page + 1);
...
const handleLoadMore = () => {
    const PageMapd =
      ...
      props.candidateByCategory.length &&
      props.candidateByCategory[0].pageCount;
    setTimeout(() => {
      if (props.candidateByCategory) {
        if (page < PageMapd) {
          setPage(page + 1);
          props.getCandidateListByCategory("Green",page,props.currentUser?props.currentUser:props.userId);
        }
        if (page === PageMapd) {
          setHasMore(false);
        }
...
export default connect(mapStateToProps, mapDispatchToProps)(CandidateGreenTable);
