...
function CandidateGreyTable(props) {
  const [page, setPage] = useState(0);
   const [hasMore, setHasMore] = useState(true);
  useEffect(() => {
    //props.getCandidateListByCategory("blue");
    props.getCandidateListByCategory("Grey",page,props.userId);
    setPage(page + 1);
...
const handleLoadMore = () => {
    ...
    setTimeout(() => {
      if (props.candidateByCategory) {
        if (page < PageMapd) {
          setPage(page + 1);
          props.getCandidateListByCategory("Grey",page,props.currentUser?props.currentUser:props.userId);
        }
        if (page === PageMapd) {
          setHasMore(false);
        }
...
export default connect(mapStateToProps, mapDispatchToProps)(CandidateGreyTable);
